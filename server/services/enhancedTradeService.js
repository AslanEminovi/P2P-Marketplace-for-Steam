...
/**
 * Updates trade cache in Redis after a successful status change
 * @param {Object} trade - The updated trade object
 * @returns {Promise<boolean>} - Success status
 */
const updateTradeCache = async (trade) => {
  if (!isRedisEnabled || !pubClient) {
    console.log("[EnhancedTradeService] Redis disabled, skipping cache update");
    return false;
  }

  try {
    const tradeId = trade._id.toString();
    const userId = trade.buyer?.toString();
    const sellerId = trade.seller?.toString();

    console.log(
      `[EnhancedTradeService] Updating Redis cache for trade ${tradeId}`
    );

    // Cache individual trade
    const tradeKey = `${TRADE_KEY_PREFIX}${tradeId}`;
    await redisConfig.setCache(tradeKey, trade, 3600); // 1 hour TTL
    ...
    // Update user's active/history trade lists
    if (userId) {
      _refreshUserTradeCache(userId);
    }

    if (sellerId && sellerId !== userId) {
      _refreshUserTradeCache(sellerId);
    }

    return true;
  } catch (error) {
    console.error("[EnhancedTradeService] Error updating trade cache:", error);
    return false;
  }
};

/**
 * Invalidates trade cache in Redis
 * @param {string} tradeId - The trade ID to invalidate
 * @param {string} userId - Optional user ID to invalidate user cache too
 * @returns {Promise<boolean>} - Success status
 */
const invalidateTradeCache = async (tradeId, userId = null) => {
  if (!isRedisEnabled || !pubClient) {
    return false;
  }
  ...
  try {
    console.log(
      `[EnhancedTradeService] Invalidating Redis cache for trade ${tradeId}`
    );

    // Delete trade from cache
    const tradeKey = `${TRADE_KEY_PREFIX}${tradeId}`;
    await redisConfig.deleteCache(tradeKey);

    // If userId provided, invalidate user's trade lists
    if (userId) {
      const userActiveKey = `${USER_TRADES_PREFIX}${userId}:active`;
      const userHistoryKey = `${USER_TRADES_PREFIX}${userId}:history`;

      await redisConfig.deleteCache(userActiveKey);
      await redisConfig.deleteCache(userHistoryKey);
    }

    return true;
  } catch (error) {
    console.error(
      "[EnhancedTradeService] Error invalidating trade cache:",
      error
    );
    return false;
  }
...
/**
 * Helper function to refresh a user's trade cache
 * @param {string} userId - User ID
 * @private
 */
const _refreshUserTradeCache = async (userId) => {
  try {
    // Delete cached trade lists to force refresh
    const userActiveKey = `${USER_TRADES_PREFIX}${userId}:active`;
    const userHistoryKey = `${USER_TRADES_PREFIX}${userId}:history`;
    ...
    console.log(
      `[EnhancedTradeService] Refreshed trade cache for user ${userId}`
    );
  } catch (error) {
    console.error(
      "[EnhancedTradeService] Error refreshing user trade cache:",
      error
    );
  }
};
