...
const itemSchema = new mongoose.Schema({
  ...
  createdAt: { type: Date, default: Date.now },
  listedAt: { type: Date }, // When the item was most recently listed
  unlistedAt: { type: Date }, // When the item was most recently unlisted
  ...
  tradeStatus: {
    type: String,
    enum: ["none", "pending", "completed", "failed", "cancelled"],
    default: "none",
  },
...
// IMPORTANT: Replace the problematic unique compound index with a non-unique one
// This allows multiple versions of the same item (listed/unlisted) but still maintains good query performance
itemSchema.index({ owner: 1, assetId: 1 });
// Add an index specifically for querying listed items efficiently
itemSchema.index({ isListed: 1, createdAt: -1 });

// Add price to price history when item is listed or price changes
itemSchema.pre("save", function (next) {
  // Set listedAt timestamp when an item becomes listed
  if (this.isModified("isListed") && this.isListed) {
    this.listedAt = new Date();
  }

  // Set unlistedAt timestamp when an item becomes unlisted
  if (this.isModified("isListed") && !this.isListed) {
  ...
  if (
    (this.isModified("price") || this.isModified("priceGEL")) &&
    this.isListed
  ) {
