  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      try {
        ...
        const response = await axios.get(`${API_URL}/user/direct-status/${userId}`, {
      ...
      if (response.data) {
        ...
        setStatus({
          isOnline: response.data.isOnline,
          lastSeen: response.data.lastSeen,
          lastSeenFormatted: response.data.lastSeenFormatted || formatLastSeen(response.data.lastSeen),
          source: 'socket'
        });
      ...
      if (retryCountRef.current < MAX_RETRIES) {
        ...
        debug(`Retrying after ${delay}ms (attempt ${retryCountRef.current + 1}/${MAX_RETRIES})`);
